...
가장 비슷하게 확인할 수 있는 플랫폼은 notion에서 다중체크와 단일체크 항목이다.
...
4. disabled : 사용자가 값을 입력할 수도없고 결과가 전송되지도 않는다. 품절처리할 때 사용
...
optgrup 태그: option group의 축약어로 카테고리 명에 해당한다. ==언어선택==과는 달리 과일/채소/육류는 선택자체가 불가능하다.
